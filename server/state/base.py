...
class BaseState:
    def __init__(self, pid: bytes, 
                 ...
                 queue_local_protos_send_ref: Callable[[BasePacket], Awaitable[None]],
                 queue_local_client_send_ref: Callable[[BasePacket], Awaitable[None]]) -> None:
        self._pid: bytes = pid
        self._change_states: Callable[[BaseState], None] = change_state_ref
        self._queue_local_protos_send: Callable[[BasePacket], Awaitable[None]] = queue_local_protos_send_ref
        self._queue_local_client_send: Callable[[BasePacket], Awaitable[None]] = queue_local_client_send_ref
    ...
    async def handle_packet(self, p: BasePacket) -> None:
        ...
        handler: Optional[Callable[[BasePacket], Awaitable[None]]] = getattr(self, handler_name, None)
        if handler:
                await handler(p)
        else:
